...
public final class ApartmentScorer {
    ...
    public static final String METHOD = """
            ...
            교육시설: 가장 가까운 초등학교 300m 이내 5점 ~ 1km 0점 + 1km 안 초·중·고 수(15곳이면 5점). 배정 학교와 무관
            생활편의: 1km 안 대형마트 수(5곳이면 4점) + 가장 가까운 대형마트 300m 이내 3점 ~ 1km 0점 + 병원 수(200곳이면 3점)
            공원: 가장 가까운 공원 200m 이내 6점 ~ 1km 0점 + 1km 안 공원 수(10곳이면 4점)
            시세 흐름: 최근 1년 평당가의 직전 1년 대비 변화율, 0%가 5점이고 ±30%에서 10점/0점. 두 기간 중 거래가 없으면 계산하지 않음.
              평형 구성이 달라지면 평당가 변화율도 달라지므로 거래 건수를 함께 본다
            기준은 반경 1km 절대 기준이다. 서울 도심처럼 시설이 밀집한 곳은 점수가 높게 몰릴 수 있다.
            """.strip();
    ...
    private static double education(Nearby n, Map<String, String> evidence) {
        ...
        double score = Math.min(schools, 15) / 15.0 * 5;
    ...
    private static double convenience(Nearby n, Map<String, String> evidence) {
        double score = Math.min(n.martCount(), 5) / 5.0 * 4 + Math.min(n.hospitalCount(), 200) / 200.0 * 3;
    ...
    private static double park(List<Spot> parks, Map<String, String> evidence) {
        ...
        double score = linear(nearest.distance(), 200, 1000, 6) + Math.min(parks.size(), 10) / 10.0 * 4;
    ...
    private static Double priceTrend(ApartmentFacts.DealSummary deals, Map<String, String> evidence) {
        ...
        return round(Math.max(0, Math.min(10, 5 + change / 6)));
    }

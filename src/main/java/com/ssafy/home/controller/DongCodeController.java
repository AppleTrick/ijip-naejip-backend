...
public class DongCodeController {
    ...
    @GetMapping("/sido")
    public ResponseEntity<CommonResponse<List<DongCodeResponse>>> getSidoList() {
        List<DongCodeResponse> sidoList = dongCodeService.getAllSido();
        log.info("시도 목록 조회 완료: {} 개", sidoList.size());
        return ResponseEntity.ok(CommonResponse.success(sidoList));
    }

    @Operation(summary = "구군 목록 조회", description = "선택한 시도에 해당하는 구군 목록을 조회합니다")
    @GetMapping("/gugun")
    public ResponseEntity<CommonResponse<List<DongCodeResponse>>> getGugunList(
        ...
        return ResponseEntity.ok(CommonResponse.success(gugunList));
    }

    @Operation(summary = "동 목록 조회", description = "선택한 시도와 구군에 해당하는 동 목록을 조회합니다")
    @GetMapping("/dong")
    public ResponseEntity<CommonResponse<List<DongCodeResponse>>> getDongList(
            @Parameter(description = "시도명 (예: 서울특별시)", required = true)
            @RequestParam("sido") String sidoName,
        ...
        return ResponseEntity.ok(CommonResponse.success(dongList));
    }

    @Operation(summary = "법정동 상세 정보 조회", description = "동코드를 이용하여 해당 법정동의 상세 정보(좌표, 주소 등)를 조회합니다")
    @GetMapping("/{code}")
    public ResponseEntity<CommonResponse<DongCodeResponse>> getDetail(
        ...
        if (dto == null) {
            return ResponseEntity.status(404)
                    .body(CommonResponse.fail("404", "해당 동코드를 찾을 수 없습니다."));
        }
